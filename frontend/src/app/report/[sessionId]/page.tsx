...
export default async function ReportPage({
  ...
  try {
    ...
    if (problem.status === 401 || problem.code === "authentication-required") {
      return <ClientReport sessionId={sessionId}>{(data) => renderReport(data)}</ClientReport>;
    }
    return (
      <div className="shell">
 ...
 *
 * Extracted so the split-domain path (G-015) renders exactly the same page
 * rather than a second, drifting copy of it.
 */
function renderReport(report: SessionReport) {
